...
/*--------------------------------------------------------------------------*
  ...
  $Id: Test5_3.cpp 763 2007-12-18 18:07:54Z nekosys $
...
INT WINAPI WinMain( HINSTANCE hInst,HINSTANCE hPrevInst,LPSTR szStr,INT nCmdshow)
{
  ...
  aiLog.Open ("../Log/Test5_3.log");

  status = aiDX.Create (hInst, _T("testDXTest5_3"), 640, 480, false, nCmdshow);
...
Test5_3::Test5_3()
{
  m_oXFile.Load ("../testResource/Tomato.x", D3DXMESH_SYSTEMMEM);
...
///
alt_t Test5_3::Render(IDirect3DDevice9 * pDevice)
{
  altCamera oCamera;
  oCamera.SetRotationWorld (GetViewRotation().x, GetViewRotation().y, GetViewRotation().z);
...
/// \return ALT_S_SUCCESS success
///
alt_t Test5_3::OnKeyDown(const altEvent & oEvent, const altUInt nKey)
{
  D3DXVECTOR3 oViewRotation = GetViewRotation();

  switch (nKey) {
    case  DIK_DOWN:
      oViewRotation.x -= 0.1f;
      break;
    case DIK_UP:
      oViewRotation.x += 0.1f;
      break;
    case DIK_LEFT:
      oViewRotation.y -= 0.1f;
      break;
    case DIK_RIGHT:
      oViewRotation.y += 0.1f;
      break;
    case DIK_Z:
      oViewRotation.z -= 0.1f;
      break;
    case DIK_X:
      oViewRotation.z += 0.1f;
      break;
