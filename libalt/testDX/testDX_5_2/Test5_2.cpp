...
/*--------------------------------------------------------------------------*
  ...
  $Id: Test5_2.cpp 763 2007-12-18 18:07:54Z nekosys $
...
INT WINAPI WinMain( HINSTANCE hInst,HINSTANCE hPrevInst,LPSTR szStr,INT nCmdshow)
{
  ...
  aiLog.Open ("../Log/Test5_2.log");

  status = aiDX.Create (hInst, _T("testDXTest5_2"), 640, 480, false, nCmdshow);
...
Test5_2::Test5_2()
{
  m_oXFile.Load ("../testResource/Corn.x", D3DXMESH_SYSTEMMEM);
...
///
alt_t Test5_2::Render(IDirect3DDevice9 * pDevice)
{
  altCamera oCamera;
...
/// \return ALT_S_SUCCESS success
///
alt_t Test5_2::OnKeyDown(const altEvent & oEvent, const altUInt nKey)
{
  D3DXVECTOR3 oViewPosition = GetViewPosition();

  switch (nKey) {
    case  DIK_LEFT:
      oViewPosition.x -= 0.1f;
      break;
    case DIK_RIGHT:
      oViewPosition.x += 0.1f;
      break;
    case DIK_DOWN:
      oViewPosition.y -= 0.1f;
      break;
    case DIK_UP:
      oViewPosition.y += 0.1f;
      break;
    case DIK_Z:
      oViewPosition.z -= 0.1f;
      break;
    case DIK_X:
      oViewPosition.z += 0.1f;
      break;
